...
### 4.2 `Click` 的兩個前提都要能關掉

`Click` 預設先等程式呼叫 `int 33h AX=4`（設游標位置），再用 `Indexed()`
（mode 13h 的 A0000）判斷畫面有沒有變。兩個前提都可能不成立：

- **游標歸誰畫**：《武士傳說》從來不叫 `AX=4`，畫面上那隻小手是它自己畫的。
  那個等待永遠不會成立，`Click` 會在跑滿預算之後回「等程式設游標位置」失敗
  ——看起來像遊戲當掉了。用 `NoCursorWait()`。
- **畫面在哪**：Tandy 模式 09h 與 CGA 模式 4 的視訊記憶體不在 `A0000`，
  取樣永遠是全 0，於是每一次點擊都回 `NoResponseError`——看起來像每一次
  都點空了。用 `Frame((*Oracle).Tandy16)` 或 `Frame((*Oracle).CGA4)`。

### 4.1 `MemOps()`：記憶體服務的軌跡

每一次 `AH=48h`／`49h`／`4Ah`／`4Bh` 記一筆：種類、區塊、要求段數、配到的段、
可用段數、這次之後的 bump 指標、成功與否、指令序號。

**配置器把已經有人住的段配出去時，症狀是程式碼被自己寫壞。** 沒有這份軌跡
就只能從被改掉的位元組往回猜寫入者；有了它，`alloc` 回的段落在映像範圍內
一眼就看得出來（`004` §1.3）。
