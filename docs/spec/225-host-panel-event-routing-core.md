...
本規格將已由使用者確認的三項 host 操作語意限縮為零依賴純核心：
...
本規格刻意不定義「未 Apply 時如何關閉面板」或其 selected scale 要保留／捨棄的 UX；也不決定
未命中 pointer 是否轉送 DOS mouse、host 文案、鍵盤快捷鍵、session／跨重啟持久化或前端 backend。
...
上述三項已確認決定完整指定了本核心處理的 state、輸入、輸出、失敗模式與邊界；未決的
持久化和 close-without-Apply 行為沒有任何欄位或 event 可被本 API 表達，故不會被實作猜補。
...
`ecac7934039ca16e9f5d2951540b6f2cb5f34985` 保存這個核心；沒有推送遠端。
