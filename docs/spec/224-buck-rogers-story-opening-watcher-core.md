...
1. `ObserveGlyphEntry` 只保留一筆 pending glyph；重疊 entry 丟棄舊 pending。
2. `ObserveInstruction` 只在 caller、SS 與 `SP+0x12` 全相符時提交 glyph。collector 只從
   sequence 1 的 exact first glyph 開始；後續 glyph／line 的每個欄位必須精確連續。
3. 五行都完成 hash 驗證後，才一次 append 五個 `StoryOpeningEvent`；partial line、錯序、hash
   miss 或 guard drift 絕不能外洩局部 event。
4. `ObserveVideoWrite` 只接受 `0CF4:1B3A`、`ES=A000` 與 nonzero `CX`。以至多 40 行的
...
- `go test ./apps/buckrogers -run TestStoryOpening -v`：五行原子完成、guard／stack／錯序／hash
  fail-closed、已證實的 `A000:AB48`／304-byte second-page span、邊界、不相交、第二頁 partial
  文字與 defensive copy 全部通過。
