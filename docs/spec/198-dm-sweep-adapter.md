...
**方向按鈕的滑鼠熱區。** §3.4 的路線是「絕對方向 → 按鈕」，而按鈕在哪裡
還沒從原版確認。已知的線索與陷阱：

- remake 那一側的 320×200 全畫面與原版**逐像素相同**
  （`internal/ui/dosfullscreen_test.go`），所以版面一致——
  但**熱區不等於圖示**，範圍要由原版的滑鼠處理常式讀出來，不能拿圖示的外框當熱區。
- **`-click-premove` 對 DM 無效**：它等的是滑鼠輪詢次數，而 DM 全程只輪詢 3 次
  （`docs/verification/` 的 DM 開機收據）。點擊要自己安排時機。

這一條屬於 G3 的第一步，不影響 §3 的契約成立——依 CLAUDE.md §0 的 G2，
READY 的判準是「可實作、未解處明寫」，不是「所有產物都做完」。
