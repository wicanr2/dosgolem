...
**門的按鈕熱區還沒解，而它是覆蓋率的瓶頸。** remake 那一側有
`World.PressDoorButton`（要隊伍面向那扇門，`F377` 的第一條分支）；
原版這一側對應的是**點地城視窗裡門上的按鈕**，座標隨門在畫面上的位置而變，
不是固定熱區。沒有按鈕的門（要鑰匙、要打破）是另一類。
在它解開之前，第 2 層的巡檢路線只有 14 步、11 格——夠當對拍載具，
但遠不是「走遍全層」。
