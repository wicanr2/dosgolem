...
{
  "map": 2,
  "start": { "x": 2, "y": 30, "facing": 3 },
  "steps": [
    { "dir": "E", "expect": { "x": 3, "y": 30, "facing": 3 } },
    { "action": "turn", "expect": { "x": 3, "y": 30, "facing": 0 } },
    { "action": "press", "expect": { "x": 3, "y": 30, "facing": 0 } },
    { "dir": "N", "expect": { "x": 3, "y": 29, "facing": 0 } }
  ],
  "checkpoints": [0, 40, 120],
  "unreachable": [[19,8],[19,9],[2,11],[2,12],[8,19],[8,20],[3,28],[4,28]]
}
```

- **`action` 有三種**，省略時是 `move`（相容只有移動的舊格式）：

  | `action` | 意思 | remake 側 | 原版側 |
  |---|---|---|---|
  | `move`（省略）| 往 `dir` 走一格 | `World.MoveParty(相對方向)` | 點對應的移動鈕 |
  | `turn` | 轉一個直角，轉完的面向在 `expect.facing` | `World.TurnParty(右?)` | 點左轉／右轉鈕 |
  | `press` | 按**正前方**那扇門的按鈕 | `World.PressDoorButton()` | 點 `(170,80)` |

  ⚠ **`turn` 一步只轉一個直角**，轉兩次就是兩步。這樣兩側都是「點一次鈕」
  對應「一步」，不必各自算最短轉法——算法不同就會在同一個檢查點上岔開。

  ⚠ **`press` 只對有按鈕的門有效**（第 2 層 25 扇裡 8 扇），而且隊伍要面向那扇門。
  產生器保證前一步已經 `turn` 到位；按了之後推進到門能過再往下走。
- **`dir` 是絕對方向**（`N`／`E`／`S`／`W`），不是「前進／後退」。
  按鈕由 `相對方向 ＝ (絕對方向 − 面向 + 4) % 4` 算出來
  （0 前進、1 右移、2 後退、3 左移），**兩側用同一個公式**。
  ⚠ **面向不是固定的**：第 2 層有六個自指傳送器，踩過去會強制轉向
  （其中 `(5,4)` 還是**絕對**旋轉），所以每一步的 `expect` 都帶 `facing`。
- **每一步都帶 `expect`**：走完之後隊伍應該在哪一格、面向哪裡。對不上就當場停，
  不要跑完整條路線才發現早就岔開了。
- **路線不必依賴傳送。** 第 2 層二十個傳送器裡**只有一個**是開著的同層傳送
  （`(4,28) → (10,28)`），而繞過所有會傳走隊伍的格子仍然到得了
  **477 / 485（98.4%）**——踩上去也只多 `(4,28)` 那一格本身。
  九個關著的傳送格是**普通可走格**，站得上去、穿得過去。
  要用傳送的話那一步標 `teleport`，`expect` 是傳送**之後**的座標與面向。
- ⚠ **門還沒解，而門決定了覆蓋率。** 「第 2 層可達 478／485」是**上界**，
  前提是所有門都開得了；實際從樓梯口出發**不開任何門只到得了 11 格**
  （remake 專案 `TestMap2ClosedDoorsDominateReachability`，
  與 `dmtool sweep route` 實跑出來的 14 步路線一致）。
  路線的動作集因此擴充成「移動 ＋ 轉向 ＋ 按門上的按鈕」（上表），
  門鈕的點擊座標是 `(170,80)`（§6）。
- **`unreachable` 是明寫的漏掉格**：六格只能經由跨層傳送走過去（踩上去隊伍
  就被送到第 1 層），`(3,28)` 的唯一通路是 `(4,28)`。路線接受漏掉，
  不為了湊 100% 去開關傳送器——那會動到感應器狀態，而感應器正是要對拍的東西。

⚠ **巡檢非招募不可**：地板感應器要隊伍裡有人才會觸發
（`SENSOR.C:474` 的 `PartyChampionCount == 0` 就跳過）。空隊伍走過壓力板
門不會開，路線走到一半就卡住——而畫面看起來只是「沒走那麼遠」。
招募流程見 `apps/dm/hall.go` 的 `MirrorRoute` ＋ `RecruitFirstChampion`。
...
1. **純測試**：結構解碼（給一段合成的 319 位元組緩衝，欄位要落在對的偏移）、
   段表算出的刻邊界等於實測的 `070a:0144`、`SweepMaximumLoad` 放得進 `AX`、
   兩條招募路線的步數與收據一致。**做完 2026-09-11**（`apps/dm` 六支）。
2. **真實資料測試**（由 `DOSGOLEM_TEST_EXE`／`DOSGOLEM_TEST_ROOT` 選配，
   走 `tools/go.sh` 的 `DOSGOLEM_ORIG` 唯讀掛載）：
   - `ReadParty` 走完招募流程讀出 `"ELIJA"`。
     **做完 2026-09-11**：`ELIJA「LION OF YAITOPYA」`，生命力 60/60、
     食物 1666、水 1667、負重 20、中毒 0。
   - 巡檢開著時跑 N 刻，生命力／食物／水不下降、中毒數維持 0。
   - **反對照**：巡檢關著時同一段跑下來，食物與水要確實下降。
     ⚠ 少了反對照就分不出「擋住了」與「這個情境本來就不會發生」。
     **做完 2026-09-11**：先把食物壓到 300，關著跑 60 刻掉到 **298**，
     接著開著跑 60 刻回到 **2048**。
     ⚠ 食物與水掉得很慢，不先壓低的話「沒掉」與「時間不夠」分不出來。
     這一輪連帶把 `FoodWaterMaximum ＝ 2048` 從**強證據**升到**已確認**：
     寫回之後跑滿 60 刻沒有被別處鉗掉。
3. **路線與停法**（同樣由私有資料選配）：
   ...
   - 停在刻的邊界時 `GameTime` 是遞增**前**的值（§3.3），用連續兩個檢查點的
     差值驗證。
4. **負重 stub**：開著時 `F309` 回 `10000`。**反對照**：取消 stub，
   同一個呼叫要走真正的計算。
   **做完 2026-09-11**：直接 `oracle.Call` 呼叫 `F309`——stub 開著回
   **10000**，取消後回 **100**（空 `CHAMPION` ＝ 力量 0 → `0×8 + 100`）。
   ⚠ 呼叫要用 `Bridge.Code` 給對的 `CS`（§6），而且值超過 16 位元要被拒絕
   （另一支測試守著）。
...
**門的按鈕熱區已解，但覆蓋率的瓶頸仍然是門。**

點擊座標是 **`(170,80)`**（`apps/dm` 的 `DoorButtonX`／`DoorButtonY`）。
來源是原版資料：門鈕在 D1C（正前方）畫完之後，原版會把**繪製框拷進點擊框表**
（remake 專案 `docs/re/66` §3），DOS 版的框是視窗座標 `x 167–174`、`y 43–51`，
地城視窗原點 `(0,33)`，中心就落在那裡。用 `dmtool dungeon doorbutton` 印得出來。

⚠ **DOS 與 ST 的框不一樣**（ST 是 `x 160–175`，中心 `(167,81)`），
而且 **ST 的那張表在 DOS 素材包裡是全零**——照它算出來的座標會落在畫面左上角。

⚠ **只有正前方的門按得到**，這與 remake 的 `World.PressDoorButton`
要求隊伍面向那扇門是同一回事。⚠ **不是每扇門都有按鈕**：第 2 層 25 扇裡
只有 8 扇，其餘要鑰匙或打破。

覆蓋率仍然受限：第 2 層兩個樓梯口，`(25,21)` 那個被**沒有按鈕的**
`(31,25)` 封住，只有 11 格；改從 `(2,30)` 出發是 **91 步 64 格（13.2%）**。
路線產生器已經改成照「不開門走得到幾格」排序自己挑。
再往上要把「轉向 ＋ 按鈕」加進路線的動作集，那是下一個切片。
