...
## 4. R38 重跑結果（2026-09-23，證偽探測選擇說）

實作後（單元測試全過）探針在 dosgolem 下回 `037Fh`，但 `HELLO.C`／
`EMPTY.C`／`FLOAT.C` 照樣 `E142`，閂鎖（`ds:0x2AC`）設下的步號與指令
與修前一致（±20 步內）。結論：**探測結果不決定數學表 index 選擇**——
閂鎖路徑與 FPU 探測無關（或探測結果傳不到選擇器）。本規格修的是真缺口
（探測語意與參照一致了），但不是 E142 的（唯一）成因。`FLOAT.C` 照爆
也同時排除「內容觸發」殘餘嫌疑。下一步見 `retro-runtime-study-private#32`
R39：找 index 0 選擇器。
