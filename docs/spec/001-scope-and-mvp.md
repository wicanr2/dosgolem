...
**結果：64,000／64,000 ＝ 100.000%**，兩張獨立抓的 oracle 都是。
重跑：`DOSGOLEM_ORIG=… tools/parity.sh <oracle.png>`。

#### 對拍要對齊的三件事

缺任何一件都不會 100%，而且**畫面看起來都是對的**：

| | 怎麼對齊 |
|---|---|
| **亂數種子** | 防拷每次問不同的題（密碼圖之一／二／三、留白哪一區）。原版那邊用固定種子版（`rich2/tools/patch_seed.py` 把 `TIMER` 內部的 `mov ah,2Ch / int 21h` 換成 `xor cx,cx / xor dx,dx`）；我們讓 `int 21h AH=2Ch` 回 CX=DX=0，效果相同，不必改 binary |
| **滑鼠位置** | 游標是遊戲自己畫的 17×27 小手，位置就是畫面內容。DOSBox 的滑鼠會持續蓋掉程式的 `AX=4`，所以兩邊都要停在同一個座標（映射 1:1） |
| **移動時機** | 程式進防拷畫面時用 `AX=4` 設一次游標位置（實測在第 42,406,064 道指令）。在那之前移動會被它蓋掉——差異一點都不會少 |

#### 已知差異：調色盤循環的相位

色號陣列 100% 相同；**調色盤**是 741／768，差的是色號 240–249。
那 10 個色在做循環動畫，兩邊相位差 2 ——相位取決於已經跑了幾個 PIT tick，
而我們的時鐘是指令數不是真時間（`docs/spec/002` §1.1 的取捨）。

**那 10 個色號在這個畫面上一個都沒用到**，所以視覺上沒有差異。
相位對齊沒有穩定意義（oracle 端的截圖時機本身有毫秒級浮動），
所以不列入驗收；真正需要時序精度時再處理（M2）。

#### 過程中修掉的四個「不會報錯」

| 症狀 | 根因 |
|---|---|
| 跑滿兩百萬道指令停在 `A303`，一個檔都沒開 | 這個 binary 是 **80186** 不是 8086（`docs/spec/002` §1.1）|
| 主控台一片空白 | 程式走 `int 21h AH=40h` 說話，不是 `AH=09h` |
| 「Please put Disk#2 in A:」 | `AH=19h` 的目前磁碟要回 C:，回 A: 會被判定成磁片版 |
| 卡在 `DX=03DA` 的兩指令迴圈 | 輪詢埠回定值，VGA 回掃等待的兩段必有一段轉不出來 |
| 文字只畫出第一個字 | 沒有 PIT 中斷，程式等的計數器（`cs:1727h`）永遠不動 |
