...
1. 在 3× 字型上驗 14px 英文字首、22px 中文字模及裁切括號；所有 bbox、逐像素透明格與跨 run 墨跡碰撞須通過。缺字、負 crop、越界與整數溢位皆須在全層預檢失敗、零輸出。
