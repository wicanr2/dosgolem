...
func executePrefix(data []byte) {
	...
	steps := 0
	for steps < 620 && !(steps > 500 && m.CPU.EIP == 0x45dd3) {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x45dd3 || steps <= 500 {
		die(fmt.Errorf("entry prefix 未在 620 steps 內完成 DOS/4GW environment 複製與第三回呼返回"))
	}
