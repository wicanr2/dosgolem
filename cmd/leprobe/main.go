...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x4cc24 && steps < 480 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x4cc24 {
		die(fmt.Errorf("entry prefix 未在 480 steps 內完成 DOS/4GW environment、第三 callback LFS 與 scan setup"))
	}
