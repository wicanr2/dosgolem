...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x45dd3 && steps < 270 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x45dd3 {
		die(fmt.Errorf("entry prefix 未在 270 steps 內完成 DOS/4GW environment、啟動緩衝區與 x87 callback return"))
	}
