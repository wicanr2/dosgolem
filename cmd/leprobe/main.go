...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x3cab3 && steps < 60 {
		if err := m.CPU.Step(); err != nil {
			die(err)
		}
		steps++
	}
	if services.Calls() != 2 || m.CPU.EIP != 0x3cab3 {
		die(fmt.Errorf("entry prefix 未在 60 steps 內讀取 ES environment cell"))
	}
	...
	fmt.Printf("entry_prefix_executed=true dos4g_branch_entered=true selector_bootstrap=true es_environment_cell=true steps=%d eip=0x%X esp=0x%X interrupts=%d eax=0x%X ebx=0x%X ecx=0x%X gs=0x%X stored_gs=0x%X stored_es=0x%X last_dx=0x%X stack_globals=0x%X,0x%X\n",
