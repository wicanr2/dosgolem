...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x460df && steps < 400 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x460df {
		die(fmt.Errorf("entry prefix 未在 400 steps 內完成 DOS/4GW environment、第一 callback 與第二 callback absolute gate"))
	}
