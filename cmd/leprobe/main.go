...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x45db0 && steps < 192 {
		if err := m.CPU.Step(); err != nil {
			die(err)
		}
		steps++
	}
	if services.Calls() != 2 || m.CPU.EIP != 0x45db0 {
		die(fmt.Errorf("entry prefix 未在 192 steps 內完成 DOS/4GW environment、啟動緩衝區與第一個 callee range gate"))
	}
	...
	fmt.Printf("entry_prefix_executed=true dos4g_branch_entered=true selector_bootstrap=true environment_path_copied=true startup_buffer_cleared=true startup_buffer_aligned=true first_near_call_entered=true callee_prologue_entered=true callee_range_gate_entered=true steps=%d eip=0x%X esp=0x%X interrupts=%d eax=0x%X ebx=0x%X ecx=0x%X gs=0x%X stored_gs=0x%X stored_es=0x%X last_dx=0x%X stack_globals=0x%X,0x%X\n",
