...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x3ca92 && steps < 56 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x3ca92 {
		die(fmt.Errorf("entry prefix 未在 56 steps 內完成 selector bootstrap"))
	}
	...
	fmt.Printf("entry_prefix_executed=true dos4g_branch_entered=true selector_bootstrap=true steps=%d eip=0x%X esp=0x%X interrupts=%d eax=0x%X ebx=0x%X gs=0x%X stored_gs=0x%X stored_es=0x%X last_dx=0x%X stack_globals=0x%X,0x%X\n",
		steps, m.CPU.EIP, m.CPU.R[cpu386.ESP], services.Calls(), m.CPU.R[cpu386.EAX], m.CPU.R[cpu386.EBX], m.CPU.Seg[cpu386.SegGS], selectorGS, storedES, uint16(m.CPU.R[cpu386.EDX]), stackA, stackB)
}
