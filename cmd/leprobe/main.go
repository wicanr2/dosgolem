...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x4cc1d && steps < 477 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x4cc1d {
		die(fmt.Errorf("entry prefix 未在 477 steps 內完成 DOS/4GW environment、第三 callback global gate 與 LFS"))
	}
