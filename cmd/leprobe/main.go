...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x3cbcc && steps < 238 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x3cbcc {
		die(fmt.Errorf("entry prefix 未在 238 steps 內完成 DOS/4GW environment、啟動緩衝區與第一個 indirect callback dispatch"))
	}
