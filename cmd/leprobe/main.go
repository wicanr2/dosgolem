...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x460e1 && steps < 401 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x460e1 {
		die(fmt.Errorf("entry prefix 未在 401 steps 內完成 DOS/4GW environment、第一 callback、第二 callback absolute gate 與 BL clear"))
	}
