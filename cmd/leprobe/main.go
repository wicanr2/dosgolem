...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x4cc10 && steps < 475 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x4cc10 {
		die(fmt.Errorf("entry prefix 未在 475 steps 內完成 DOS/4GW environment、第三 callback stack frame 與 global gate"))
	}
