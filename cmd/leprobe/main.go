...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x460fb && steps < 440 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x460fb {
		die(fmt.Errorf("entry prefix 未在 440 steps 內完成 DOS/4GW environment 與第二 callback x87 self-test helper"))
	}
