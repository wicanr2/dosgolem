...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x45dcf && steps < 235 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x45dcf {
		die(fmt.Errorf("entry prefix 未在 235 steps 內完成 DOS/4GW environment、啟動緩衝區與第一個 callback pointer gate"))
	}
