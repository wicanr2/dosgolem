...
func executePrefix(data []byte) {
	...
	steps := 0
	callbackReturns := 0
	for callbackReturns < 2 && steps < 452 {
		if m.CPU.EIP == 0x45dd6 {
			callbackReturns++
			if callbackReturns == 2 {
				break
			}
		}
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x45dd6 || callbackReturns != 2 {
		die(fmt.Errorf("entry prefix 未在 452 steps 內完成 DOS/4GW environment 與第二 callback record status write"))
	}
