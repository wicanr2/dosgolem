...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x4cc03 && steps < 470 {
		if err := m.CPU.Step(); err != nil {
			die(err)
		}
		steps++
	}
	if services.Calls() != 2 || m.CPU.EIP != 0x4cc03 {
		die(fmt.Errorf("entry prefix 未在 470 steps 內完成 DOS/4GW environment、第三 callback prologue 與 PUSH FS"))
	}
