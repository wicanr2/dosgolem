...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x4cc51 && steps < 491 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x4cc51 {
		die(fmt.Errorf("entry prefix 未在 491 steps 內完成 DOS/4GW environment 與第一次 Watcom _nmalloc 返回"))
	}
