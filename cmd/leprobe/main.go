...
func executePrefix(data []byte) {
	...
	steps := 0
	for m.CPU.EIP != 0x45dd6 && steps < 271 {
	...
	if services.Calls() != 2 || m.CPU.EIP != 0x45dd6 {
		die(fmt.Errorf("entry prefix 未在 271 steps 內完成 DOS/4GW environment、x87 callback 與 record status write"))
	}
