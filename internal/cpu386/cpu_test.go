...
func TestStoreRegisterToStackDisp32(t *testing.T) {
	...
	c := New(mem)
	c.R[ESP], c.R[EDX], c.EFlags = 0x40, 0x12345678, CF|ZF|OF
	c.Seg[SegSS] = 0x168
	c.SetDescriptor(0x168, Descriptor{Limit: 0x7f, Writable: true})
	if err := c.Step(); err != nil || !bytes.Equal(mem[0x30:0x34], []byte{0x78, 0x56, 0x34, 0x12}) || c.R[ESP] != 0x40 || c.R[EDX] != 0x12345678 || c.EFlags != CF|ZF|OF {
		t.Fatalf("stack disp32 store=% X ESP=%X EDX=%X flags=%X err=%v", mem[0x30:0x34], c.R[ESP], c.R[EDX], c.EFlags, err)
	}

	mem = testBus(make([]byte, 0x40))
	copy(mem, []byte{0x89, 0x84, 0x24, 0x10, 0, 0, 0})
	c = New(mem)
	c.R[ESP], c.R[EAX] = 0x20, 0x12345678
	c.Seg[SegSS] = 0x168
	c.SetDescriptor(0x168, Descriptor{Limit: 0x3f})
	if err := c.Step(); err == nil || !bytes.Equal(mem[0x30:0x34], []byte{0, 0, 0, 0}) {
		t.Fatalf("read-only stack store=% X err=%v", mem[0x30:0x34], err)
	}
}
...
func TestCompareEDXWithEAXDisp8Dword(t *testing.T) {
	for _, test := range []struct {
		name  string
		left  uint32
		right uint32
		...
		t.Run(test.name, func(t *testing.T) {
			mem := testBus(make([]byte, 0x40))
			copy(mem, []byte{0x3b, 0x50, test.delta})
			binary.LittleEndian.PutUint32(mem[test.addr:test.addr+4], test.right)
			c := New(mem)
			c.R[EAX], c.R[EDX] = 0x20, test.left
			c.Seg[SegDS] = 0x160
			c.SetDescriptor(0x160, Descriptor{Limit: 0x3f})
			before := append([]byte(nil), mem...)
			if err := c.Step(); err != nil || c.R[EAX] != 0x20 || c.R[EDX] != test.left || !bytes.Equal(mem, before) || c.EFlags&(CF|PF|AF|ZF|SF|OF) != test.flags {
				t.Fatalf("CMP EDX=%X EAX=%X right=%X flags=%X memoryChanged=%v err=%v", c.R[EDX], c.R[EAX], test.right, c.EFlags, !bytes.Equal(mem, before), err)
			}
		})
	}

	mem := testBus{0x3b, 0x50, 4}
	c := New(mem)
	c.R[EAX], c.R[EDX], c.EFlags = 0x20, 7, CF|ZF|OF
	c.Seg[SegDS] = 0x160
	c.SetDescriptor(0x160, Descriptor{Limit: 3})
	if err := c.Step(); err == nil || c.R[EAX] != 0x20 || c.R[EDX] != 7 || c.EFlags != CF|ZF|OF {
		t.Fatalf("out-of-range CMP EAX=%X EDX=%X flags=%X err=%v", c.R[EAX], c.R[EDX], c.EFlags, err)
	}
}
