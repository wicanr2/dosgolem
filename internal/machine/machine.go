...
// `docs/spec/004` §5.2。
//
// ⚠ **這個校準與指令數時鐘的標定沒有互相推導過，兩者的隱含速度不一致。**
// 指令數那邊是 `StepsPerSecond()`（11.58 M 指令／秒，`docs/spec/190`）；
// 週期這邊要換算得先知道平均每道指令幾個週期，而那隨程式的指令混合走
// ——`determinism_test` 的 spinner 量到 9.67（換算 3.41 M），但那段是
// `in`／記憶體／跳躍的三道迴圈，不代表真實程式。
//
// 目前兩個時鐘是獨立模型，切換會讓相位移位（`docs/spec/004` §5 明說了
// 這件事），所以不一致本身不是錯的。要讓它們對得起來得先量一份有代表性
// 的指令混合。
