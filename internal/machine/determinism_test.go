...
func spinner(t *testing.T) *Machine {
	...
	// ⚠ **`int 1Ch` 要指到一個真的會回來的地方。** BIOS 的 int 08h 預設處理
	// 推完 0040:006C 之後會轉呼 int 1Ch，而每個向量的預設 stub 是
	// `CD n; CF`——INT 指令平常由 DOS 服務層攔下來，這裡沒有 DOS，
	// 於是 `CD 1C` 又跳回同一段 stub，一路遞迴而且 IF 再也不會被打開。
	// 症狀是「只送得出第一次計時器中斷」，看起來像計時器壞了。
	const iretAt = 0x0400 // 0040:0000 之外的低位空白處
	m.Write8(iretAt, 0xCF)
	m.Write16(0x1C*4, uint16(iretAt&0x0F))
	m.Write16(0x1C*4+2, uint16(iretAt>>4))
	return m
}
