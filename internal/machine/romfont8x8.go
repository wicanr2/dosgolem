// Code generated from font8x8_basic.h (Daniel Hepper, Public Domain); the table below is generated, initROMFont is hand-written.
// 來源：https://github.com/dhepper/font8x8 的 font8x8_basic.h（U+0000–U+007F，Public Domain）。
// 原檔「最低位元是最左邊的點」，這裡每個 byte 已反轉成 IBM ROM 字型的「最高位元在最左邊」。
// 用途見 `docs/spec/199-rom-font-8x8`。

package machine

// romFont8x8 是放在 F000:FA6E 的 8×8 字型，字元 0–127，每字 8 列。
var romFont8x8 = [128][8]byte{
...
// initROMFont 把 8×8 字型放到 F000:FA6E，IBM PC 相容 BIOS 放 CGA 字型前半的固定位址
// （`docs/spec/199-rom-font-8x8`）。BGI 的 DEFAULT_FONT 有一部分字元直接讀這裡；
// 讀到 0 的話那些字元畫成空白，而字母照常——看起來像程式漏印。
func (m *Machine) initROMFont() {
	const base = 0xFFA6E
	for ch := range romFont8x8 {
		copy(m.Mem[base+ch*8:base+ch*8+8], romFont8x8[ch][:])
	}
}
