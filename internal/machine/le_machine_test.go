...
func TestFD2EntryPrefixWhenProvided(t *testing.T) {
	...
	for steps := 0; m.CPU.EIP != 0x3caeb && steps < 60; steps++ {
		if err := m.CPU.Step(); err != nil {
			t.Fatal(err)
		}
	}
	if services.Calls() != 2 || m.CPU.EIP != 0x3caeb {
		t.Fatalf("entry did not scan command-tail spaces: calls=%d EIP=%X", services.Calls(), m.CPU.EIP)
	}
	if m.CPU.R[cpu386.EAX] != 0x20 || m.CPU.R[cpu386.EBX] != 0x160 || m.CPU.Seg[cpu386.SegGS] != 0x20 || m.CPU.EFlags&cpu386.ZF == 0 {
	...
	if uint8(m.CPU.R[cpu386.EAX]) != 0x20 || m.CPU.R[cpu386.EDI] != 0x81 || m.CPU.EFlags&cpu386.DF != 0 {
		t.Fatalf("space scan EAX=%X EDI=%X flags=%X", m.CPU.R[cpu386.EAX], m.CPU.R[cpu386.EDI], m.CPU.EFlags)
	}
