...
//
// 少了這一層，程式把分頻值設成 100（約 11.9 kHz）之後中斷仍然
// 每 165,000 道指令來一次——**而且不會報錯**，只是語音一秒要跑
// 一億八千萬道指令。
func TestPITChannel0ChangesTheInterruptRate(t *testing.T) {
	m := New()
	if m.IRQ0Every != DefaultIRQ0Every {
		t.Fatalf("開機的 IRQ0Every 是 %d，應該是 %d", m.IRQ0Every, DefaultIRQ0Every)
	}
	...
	m.Out8(0x40, 100>>8)
	want := uint64(DefaultIRQ0Every) * 100 / 65536
	if m.IRQ0Every != want {
	...
	m.Out8(0x40, 0)
	if m.IRQ0Every != DefaultIRQ0Every {
		t.Errorf("分頻值 0 沒有還原成 %d（現在 %d）", DefaultIRQ0Every, m.IRQ0Every)
	}
}
...
func TestPITOtherChannelsDoNotChangeIRQ0(t *testing.T) {
	...
	m.Out8(0x42, 0x12)
	if m.IRQ0Every != DefaultIRQ0Every {
		t.Errorf("通道 2 的設定改到了 IRQ0Every（%d）", m.IRQ0Every)
	}
}
...
func TestSnapshotRestoresSpeakerAndPIT(t *testing.T) {
	...
	m.Out8(0x40, 0)
	m.Out8(0x61, 0x02)
	if m.IRQ0Every == DefaultIRQ0Every || len(m.Speaker) == 0 {
		t.Fatal("前置動作沒生效")
	}
	m.Restore(snap)
	if m.IRQ0Every != DefaultIRQ0Every {
