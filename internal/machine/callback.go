...
//
// ⚠ **F2–F6 已經被服務型中斷的 trampoline 佔走**（`machine.initVectors`）：
// F2 ＝ int 21h、F3 ＝ int 10h、F4 ＝ int 15h、F5 ＝ XMS、F6 ＝ EMS。
// 撞號的話回呼結束會被當成一次視訊服務，**而且不會報錯**——
// 回呼永遠不收尾，機器卡在一個回不來的遠呼叫裡。
const IntCallbackReturn = 0xF9
