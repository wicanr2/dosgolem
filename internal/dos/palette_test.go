...
func TestMode13LoadsDefaultVGA16DAC(t *testing.T) {
	m, d := newTest(t)
	for i := range m.DAC {
		m.DAC[i] = 1
	}
	m.CPU.R[cpu.AX] = 0x0013
	...
	for i, rgb := range want {
		got := [3]uint8{m.DAC[i*3], m.DAC[i*3+1], m.DAC[i*3+2]}
		if got != rgb {
			t.Errorf("DAC[%d] ＝ %v，預期 %v", i, got, rgb)
		}
	}
	if got := [3]uint8{m.DAC[48], m.DAC[49], m.DAC[50]}; got != [3]uint8{1, 1, 1} {
		t.Errorf("本規格只載入前 16 色，DAC[16] 不應被改寫：%v", got)
	}
}
